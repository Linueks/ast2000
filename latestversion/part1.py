...
def simulate_launch(number_of_partitions, partition_force, number_of_particles_per_partition_per_second):
    ...
    for t in range(number_of_time_steps):
        ...
        escape_velocity = 11269.957#Launch needed from part 3 #np.sqrt(2 * gravitational_constant * home_planet_mass / home_planet_radius)#radial_position)
...
def calculate_position_relative_to_sun(launch_time, distance_from_planet_center, escape_velocity):
    ...
    init_rot = np.array((-satellite_position_relative_to_planet[1], satellite_position_relative_to_planet[0]))
    final_rot_unit = init_rot / np.linalg.norm(init_rot)
    final_rot_vel = final_rot_unit * w * distance_from_planet_center
    final_vel = escape_velocity * satellite_position_relative_to_planet/np.linalg.norm(satellite_position_relative_to_planet) + final_rot_vel + planet_velocity
    ...
    final_vel *= ms_to_AUyr


    return initial_satellite_position_relative_to_star, satellite_position_relative_to_star, final_vel, satellite_position_relative_to_planet*m_to_AU
...
#print simulate_engine(100000, 1000, 1e-12, 1e-6, 10000)
...
force_wanted = gravity_force_at_surface + 1000

engine_partitions_needed = force_wanted/partition_force
print engine_partitions_needed
plot(100, 1000, 1e-12, 1e-6, 10000)

#initial_satellite_position, satellite_position_after_launch, satellite_velocity_relative_to_planet, dritt = calculate_position_relative_to_sun(1105.029752 , 10794959.775989799, 11270.004104761132)
#print calculate_position_relative_to_sun(1104.959999999962, 10641423.142761754, 10789.272032602184)

#print initial_satellite_position

#print simulate_launch(engine_partitions_needed, partition_force, particles_per_sec_per_partition)


#star_system.engine_settings(partition_force, engine_partitions_needed, particles_per_sec_per_partition, fuel_mass, 1105.029752, initial_satellite_position, 11.3)
#print star_system.mass_needed_launch(satellite_position_after_launch, test=True)
#print satellite_position_after_launch

#engine_settings(partition_force, engine_partitions_needed, particles_per_sec_per_partition, fuel_mass, 1106, initial_satellite_position, 0)
#mass_needed_launch(star_system, satellite_position_after_launch, test=True)
